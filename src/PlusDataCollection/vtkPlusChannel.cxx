...
//----------------------------------------------------------------------------
PlusStatus vtkPlusChannel::AddParameterDataSource(vtkPlusDataSource* aSource)
{
	if (aSource == NULL)
	{
		LOG_ERROR("Trying to add null parameter data source to channel.");
		return PLUS_FAIL;
	}

	for (DataSourceContainerConstIterator it = this->ParameterDataSources.begin(); it != this->ParameterDataSources.end(); ++it)
	{
		if (it->second == aSource)
		{
			// source has been already added
	...
	this->ParameterDataSources[aSource->GetId()] = aSource;
	this->ParameterDataSources[aSource->GetId()]->Register(this);

	return PLUS_SUCCESS;
}

//----------------------------------------------------------------------------
PlusStatus vtkPlusChannel::RemoveParameterDataSource(const std::string& sourceId)
{
	if (sourceId.empty())
	{
		LOG_ERROR("vtkPlusChannel::RemoveParameterDataSource failed: sourceId is invalid");
		return PLUS_FAIL;
	}

	for (DataSourceContainerIterator it = this->ParameterDataSources.begin(); it != this->ParameterDataSources.end(); ++it)
	{
		if (it->second->GetId() == sourceId)
		{
			this->ParameterDataSources.erase(it);
			return PLUS_SUCCESS;
		}
	}
...
//----------------------------------------------------------------------------
PlusStatus vtkPlusChannel::GetParameterDataSource(vtkPlusDataSource*& aSource, const std::string& sourceId)
{
	if (sourceId.empty())
	{
		LOG_ERROR("vtkPlusChannel::GetParameterDataSource failed: sourceId is invalid");
		return PLUS_FAIL;
	}
...
//----------------------------------------------------------------------------
PlusStatus vtkPlusChannel::RemoveParameterDataSources()
{
	this->ParameterDataSources.clear();

	return PLUS_SUCCESS;
}

//----------------------------------------------------------------------------
PlusStatus vtkPlusChannel::GetParameters(std::map<std::string, std::string>& parameters)
{
	vtkPlusDataSource* aSource(NULL);
	if (this->GetParameterDataSource(aSource, "GetParameters") == PLUS_SUCCESS)
	{
		std::string propertyValue = aSource->GetCustomProperty("Processed");
		if (propertyValue.compare("Read") == 0)
		{
			aSource->GetCustomProperties(parameters);
			//testcode
			LOG_INFO("Test vtkPlusChannel::GetParameters");

			aSource->SetCustomProperty("Processed", "Sent");
			//LOG_INFO("GetParameters parameters size: " << parameters.size());
			this->RemoveParameterDataSource("GetParameters");//TODO: Delete parameter data source after it have been sent? If so, need to update function name
